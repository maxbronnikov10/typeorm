 ...
 */
export class PostgresJsQueryRunner
    ...
    implements QueryRunner
{
    ...
    async stream(
        ...
        const databaseConnection = await this.connect()
        this.driver.connection.logger.logQuery(query, parameters, this)
        const stream = (await databaseConnection
            .unsafe(query, parameters)
            .readable()) as ReadStream
        if (onEnd) stream.on("end", onEnd as () => void)
        if (onError) stream.on("error", onError as (err: Error) => void)
