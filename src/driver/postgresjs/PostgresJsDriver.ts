...
/**
 * Organizes communication with PostgreSQL via postgres.js.
 */
export class PostgresJsDriver implements Driver {
    ...
    loadStreamDependency() {
        throw new TypeORMError(
            `Postgres.js driver uses postgres.js readable() streams. pg-query-stream is not supported.`,
        )
    }
    ...
    protected async createPool(
        ...
        const baseOptions: Record<string, any> = {
            ...
            connect_timeout: options.connectTimeoutMS
                ? Math.ceil(options.connectTimeoutMS / 1000)
                : undefined,
            onnotice: options.logNotifications
                ? (notice: { message?: string }) => {
                      const message =
                          notice?.message ?? JSON.stringify(notice)
                      if (message) {
                          this.connection.logger.log("info", message)
                      }
                  }
                : undefined,
        }
        const extraOptions = options.extra || {}
        const connectionOptions = Object.assign({}, baseOptions, extraOptions)
        const extraConnection = extraOptions.connection as
            | Record<string, any>
            | undefined
        if (applicationName || extraConnection) {
            connectionOptions.connection = Object.assign(
                {},
                extraConnection || {},
        ...
        if (options.parseInt8 !== undefined) {
            logger.log(
                "warn",
                "Attempted to set parseInt8 option, but postgres.js does not support this option. This option will be ignored.",
            )
        }

        const sql = credentials.url
            ? this.postgres(credentials.url, connectionOptions)
            : this.postgres(connectionOptions)
        const pool = new PostgresJsPool(sql)
        ...
        return new Promise((ok, fail) => {
            pool.connect((err: any, _connection: any, release: Function) => {
...
class PostgresJsPoolConnection {
    private readonly reserved: any
    private readonly errorHandler?: (error: any) => void

    constructor(reserved: any, errorHandler?: (error: any) => void) {
    ...
    async query(query: string, parameters?: any[]) {
        try {
            const result = await this.reserved.unsafe(query, parameters)
    ...
    unsafe(query: string, parameters?: any[]) {
        return this.reserved.unsafe(query, parameters)
    }
...
class PostgresJsPool {
    ...
    on(event: string, handler: (error: any) => void) {
        if (event === "error") {
    ...
    connect(
        callback: (err: any, connection?: any, release?: Function) => void,
    ) {
        this.sql
            .reserve()
            .then((reserved: any) => {
                const connection = new PostgresJsPoolConnection(
                    reserved,
                    this.errorHandler,
                )
                callback(
    ...
    end(callback?: (err?: any) => void) {
        return this.sql.end().then(
            () => callback?.(),
